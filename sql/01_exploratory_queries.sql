...
SELECT song_name, artist_name, platform
...
SELECT song_name, sec_played
FROM streaming_history
WHERE
    sec_played > 120
    AND artist_name = 'LiSA';
...
SELECT count(
        CASE
            WHEN artist_name = 'RADWIMPS' THEN 1
        END
    )
FROM streaming_history;
...
SELECT artist_name, sum(sec_played)
FROM streaming_history
GROUP BY
    artist_name;
...
SELECT platform, artist_name, COUNT(song_name)
FROM streaming_history
GROUP BY
    artist_name,
    platform
...
SELECT
    ...
    COUNT(
        CASE
            WHEN incognito_mode = 1 THEN 1
        END
    ) AS incognito,
    COUNT(
        CASE
            WHEN incognito_mode = 0 THEN 1
        END
    ) AS non_incognito
FROM streaming_history
GROUP BY
...
SELECT song_name, sec_played, RANK() OVER (
        ORDER BY song_name ASC, sec_played DESC
    )
FROM streaming_history;
...
SELECT
    RANK() OVER (
        partition by
            platform
        ORDER BY sec_played desc, artist_name ASC
    ),
    platform,
    artist_name,
    song_name,
